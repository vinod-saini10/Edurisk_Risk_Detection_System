...
export default function PreviousPage() {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const { data } = await getPrevious({ name: name.trim(), email: email.trim() });
      setResult(data);
    } catch (err) {
      setError(err.response?.data?.error || "No record found or server error.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ maxWidth: 620, margin: "0 auto", padding: "2.5rem 1.5rem" }}>
      ...
      <form onSubmit={handleSearch} style={{ display: "flex", flexDirection: "column", gap: "1rem", marginBottom: "2rem" }}>
        {[
          { label: "Full Name", val: name, set: setName, type: "text", ph: "Your registered name" },
          { label: "Email", val: email, set: setEmail, type: "email", ph: "Your registered email" },
        ].map(f => (
          <div key={f.label}>
            <label style={{ display: "block", fontSize: "0.82rem", fontWeight: 600, color: "#94a3b8", marginBottom: 6 }}>{f.label}</label>
            <input
        ...
        {error && (
          <div style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 10, padding: "0.8rem 1rem", color: "#f87171", fontSize: "0.875rem" }}>
            ⚠ {error}
          </div>
        )}
      ...
      {result && (
        <div style={{
          ...
          borderRadius: 18, padding: "1.8rem", animation: "fadeIn 0.4s ease",
        }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.2rem", flexWrap: "wrap", gap: 8 }}>
            <div>
              <h3 style={{ fontWeight: 700, color: "#f1f5f9", fontSize: "1.1rem" }}>{result.name}</h3>
              <p style={{ color: "#64748b", fontSize: "0.82rem" }}>{result.email}</p>
            </div>
          ...
          <div style={{
            ...
            padding: "1rem", background: "rgba(79,110,247,0.08)",
            borderRadius: 12, border: "1px solid rgba(79,110,247,0.2)",
          }}>
            <div style={{ textAlign: "center", flex: 1 }}>
              <div style={{ fontSize: "2rem", fontWeight: 800, color: "#4f6ef7" }}>
                {Math.round(result.predicted_score)}
              </div>
              <div style={{ fontSize: "0.75rem", color: "#64748b" }}>Predicted Score</div>
            </div>
            <div style={{ width: 1, background: "rgba(255,255,255,0.08)" }} />
            <div style={{ textAlign: "center", flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
              <RiskBadge level={result.risk_level} />
              <div style={{ fontSize: "0.75rem", color: "#64748b", marginTop: 6 }}>Risk Level</div>
            </div>
          </div>

          <DataRow label="Attendance" value={`${result.attendance}%`} />
          <DataRow label="Study Hours/Day" value={`${result.study_hours} hrs`} />
          ...
          <DataRow label="Recorded At" value={new Date(result.created_at).toLocaleString()} />

          <button
            onClick={() => nav("/dashboard", { state: { result } })}
            style={{
              marginTop: "1.2rem", width: "100%",
              background: "rgba(79,110,247,0.15)", color: "#7c9ef7",
              border: "1px solid rgba(79,110,247,0.3)", borderRadius: 10,
              padding: "0.75rem", fontWeight: 600, cursor: "pointer", fontSize: "0.9rem",
            }}>
            📊 View on Dashboard
          </button>
        </div>
