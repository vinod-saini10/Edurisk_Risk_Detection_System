...
export default function BulkUploadPage() {
    ...
    try {
      ...
      setError(
        err.response?.data?.error ||
          "Failed to process CSV. Ensure format is correct.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const downloadResults = () => {
    ...
    const csvContent =
      header +
      results
        .map((r) => `${r.name},${r.email},${r.score},${r.risk}`)
        .join("\n");
    ...
    <div style={{ maxWidth: 900, margin: "0 auto", padding: "3rem 1.5rem" }}>
      <div style={{ marginBottom: "2rem" }}>
        ...
        <p style={{ color: "var(--text-secondary)" }}>
          Upload a CSV file with student academic data for instant bulk
          analysis.
        </p>
      </div>
      ...
      <div
        className="card"
        style={{ padding: "2.5rem", textAlign: "center", marginBottom: "2rem" }}
      >
        <div style={{ marginBottom: "1.5rem" }}>
          <div style={{ fontSize: "3rem", marginBottom: "0.5rem" }}>📄</div>
          <p style={{ fontSize: "1rem", fontWeight: 600 }}>Select a CSV file</p>
          <p style={{ fontSize: "0.8rem", color: "var(--text-secondary)" }}>
            Required columns: attendance, study_hours, previous_marks,
        ...
        {success && (
          <p
            style={{ color: "#10b981", marginTop: "1rem", fontSize: "0.9rem" }}
          >
            ✅ {success}
          </p>
        )}
